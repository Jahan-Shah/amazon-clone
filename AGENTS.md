...
No test command is configured yet, so there is no required test gate. Testing
is opt-in: run `/tests` or `$tests` to add a runner and update this section
with the real test commands.
