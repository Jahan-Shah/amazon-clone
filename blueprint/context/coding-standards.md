...
> Your conventions. Tuned by `/onboard` to the detected stack: Next.js 16
> (App Router, root `app/` directory, no `src/`), TypeScript strict, Tailwind
> CSS v4, pnpm. No database, auth, or component library is installed yet;
> sections that depend on those are marked `> TODO`.
...
- Pages: `app/[route]/page.tsx` (App Router lives at the repo root `app/`, no `src/`)
- Server Actions: `actions/[feature].ts`
- Types: `types/[feature].ts`
- Lib/Utils: `lib/[utility].ts`
- Imports use the `@/*` path alias, which maps to the repo root (`./`)
...
- No inline styles
- > TODO: no component library is installed yet. Add one deliberately (for
>   example shadcn/ui) or keep hand-built components; record the choice here.
- > TODO: theming (dark/light) is unconfirmed; the scaffold only ships a
>   `prefers-color-scheme` default in `globals.css`.

## Database

> TODO: no database or ORM is configured yet. Record the choice, migration
> command, and sync check here when a data layer is added.

## Data Fetching

- Server components fetch data directly; client components go through Server Actions
- > TODO: no validation library is installed yet. Validate all untrusted input
>   at the boundary (forms, route handlers, Server Actions); record the chosen
>   library here.
- Scope every user-owned query by the authenticated user id from the server
  session; never trust a client-supplied user id
...
Stack binding: this TypeScript app should use Vitest with `vi.mock()` for
external dependencies (database, auth, network) and `vi.useFakeTimers()` for
